...
// never match any goal's status field, making the filter permanently empty.
// The overflow/overdrive concept is a display detail handled inside MilestoneNode,
// not a filterable lifecycle status.
...
export default function GoalsPage() {
  const {
    ...
    fuelGoal,
    createGoal,
    // FIX #1: Destructure removeGoalFromState from the hook — it was called in
    // handleDeleteGoal but never imported, causing a ReferenceError on every delete.
    removeGoalFromState,
    // Currency
    displayCurrency,
    setDisplayCurrency,
  ...
  // FIX #3: Filter and count on `g.status` — the enriched goal objects returned
  // by the service carry a `status` field, not `derivedStatus`. Filtering on
