...
if (!process.env.GEMINI_API_KEY) {
  logger.warn('WARNING: GEMINI_API_KEY missing from environment context initialization variables.');
}
...
2. "cleanDescription" strips currency symbols and prices — keep just the merchant name or transaction objective.
`;
...
- Currency values must be displayed exactly as provided (e.g., Rs. or base amounts).
...
- Net Savings: Rs. ${Number(summary.netSavings || 0).toFixed(2)}
...
`;
    ...
    logger.info(`Forwarding financial context data to ${CORE_MODEL} engine...`);
...
- Income: Rs. ${Number(metrics.totalIncome || 0).toFixed(2)}
- Expenses: Rs. ${Number(metrics.totalExpenses || 0).toFixed(2)}
...
`;
    ...
    logger.info(`Sending account optimization matrices to ${CORE_MODEL} context...`);
